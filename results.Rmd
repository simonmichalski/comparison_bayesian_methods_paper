...
```
<br>
With a decision threshold of 3, `r proportion_to_percentage(fp_savage_dickey_bf_3)``\%` of Savage-Dickey BFs were false positives. With a decision threshold of 1/3 for a negative and 3 for a positive effect, `r proportion_to_percentage(fp_directional_bf_3)``\%` of dBFs were false positives. With 1/10 and 10 as directional decision thresholds, `r proportion_to_percentage(fp_directional_bf_10)``\%` of dBFs were false positives, with 1/30 and 30, `r proportion_to_percentage(fp_directional_bf_30)``\%` of dBFs  were false positives, and with 1/100 and 100, `r proportion_to_percentage(fp_directional_bf_100)``\%` of dBFs  were false positives. When applying the HDI against zero decision rule with a 80`\%` HDI, `r proportion_to_percentage(fp_hdi_80)``\%` of decisions were false positives, with a 90`\%` HDI, `r proportion_to_percentage(fp_hdi_90)``\%` of decisions were false positives, with a 95`\%` HDI, `r proportion_to_percentage(fp_hdi_95)``\%` of decisions were false positives, and with a 99`\%` HDI, `r proportion_to_percentage(fp_hdi_99)``\%` of decisions were false positives. With P(effect > 0) > .95 for a positive and P(effect > 0) < .05 for negative effect, `r proportion_to_percentage(fp_p_effect_95)``\%` of P(effect > 0) proportions were false positives, with > .975 and < .025, `r proportion_to_percentage(fp_p_effect_975)``\%` of P(effect > 0) proportions were false positives, and with > .99 and < .01, `r proportion_to_percentage(fp_p_effect_99)``\%` of P(effect > 0) proportions were false positives. For each inference method, false positive results were lower, the stricter the decision threshold was. False positive results descriptively increased with increasing prior width for all decision thresholds of the dBF, P(effect > 0), and the HDI against zero decision rule, whereas false positive Savage-Dickey BFs did not increase with wider priors. With wider priors and relatively stricter thresholds, false positive results were descriptively higher within samples from the highest population effect standard deviation.
<br>
```
\begin{table}[H]
