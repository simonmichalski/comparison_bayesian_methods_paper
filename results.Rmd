...
cor_s_log_k_overall <- mean(data_cor_s_log_k$correlation)
cor_s_log_k_effect_sd_0_2 <- mean(data_cor_s_log_k$correlation[data_cor_s_log_k$s_log_k_sd == 0.2])
cor_s_log_k_effect_sd_0_51 <- mean(data_cor_s_log_k$correlation[data_cor_s_log_k$s_log_k_sd == 0.51])
cor_s_log_k_effect_sd_0_81 <- mean(data_cor_s_log_k$correlation[data_cor_s_log_k$s_log_k_sd == 0.81])

data_cor_log_k <- summarize(group_by(df_recovery, s_log_k_sd, prior_sd, sample), 
                            correlation = cor(true_log_k, median_log_k))
cor_log_k_overall <- mean(data_cor_log_k$correlation)
...
The mean correlation between true and estimated subject-level s`\textsubscript{log(\textit{k})}` parameters within samples from the small population effect standard deviation was `\textit{r}` = `r remove_leading_zero(round(cor_s_log_k_0_2, 2))`, within samples from the medium population effect standard deviation `\textit{r}` = `r remove_leading_zero(round(cor_s_log_k_0_51, 2))`, and within samples from the large population effect standard deviation `\textit{r}` = `r remove_leading_zero(round(cor_s_log_k_0_81, 2))`. THe mean correlation over all samples was `\textit{r}` = `r remove_leading_zero(round(cor_s_log_k_overall, 2))`. The mean correlation between true and estimated subject-level log(`\textit{k}`) parameters within samples was `\textit{r}` = `r remove_leading_zero(round(cor_log_k_overall, 2))`.

-> partial pooling stats and multiplot

-> table: true subj param SD | group-level SD | subj-level estimate SD | Partial pooling (SD reduction)
- 4 rows: log_k + s_log_k (3 effect SDs)
...
The mean variance difference between true subject-level parameters and subject-level estimates within samples was XX for the large population effect standard deviation, XX for the medium population effect standard deviation, and XX for the small population effect standard deviation (???).`\\`
