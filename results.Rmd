...
#df_sim_thres_prior <- readRDS(file.path("results", "sim_thres_per_effect_sd.rds"))
...
get_fp_per_prior_sd <- function(column_pos, column_neg){
  if (!all(c(column_pos, column_neg) %in% colnames(df_results))){
...
get_prior_sensitivity <- function(column){
  ...
  only_fp_samples <- 0
  for (i in seq(from = 1, by = 8, length.out = length(column)/8)){
    if (any(column[seq(i, i+7)] == 1)){
      diverging_samples <- diverging_samples + 1
    }
    if (all(column[seq(i, i+7)] == 1) == TRUE){
      only_fp_samples <- only_fp_samples + 1
    }
  }
  proportion <- only_fp_samples / diverging_samples
  return(proportion)
}
...
`\section{Results}`  
Note that the intervals between the prior standard deviations are not equal, as they were chosen to cover the most plausible prior standard deviations between zero and 2.5. Hence, overall means cannot be interpreted continuously as averages of all possible prior standard deviations between 0.05 and 2.5, but as averages of discrete prior variations selected in this study.
...
\begin{table}[H]
    ...
    \midrule
    s\textsubscript{log(\textit{k})} $\sim \mathcal{N}$(0,0.2) & \centering `r round_and_format(mean_sd_true_0_2)` & \centering `r round_and_format(sd_sd_true_0_2)` & \centering `r round_and_format(mean_sd_estimates_0_2)` & \centering `r round_and_format(sd_sd_estimates_0_2)` & \centering `r proportion_to_percentage(mean_shrinkage_0_2)` & \centering `r  proportion_to_percentage(sd_shrinkage_0_2)`\arraybackslash\\
...
\begin{table}[H]
    ...
    \midrule
    \multicolumn{1}{c}{Decision rule} & \multicolumn{8}{c}{Proportion of false positive results per decision rule and prior \textit{SD} (\%)}\\
    \cmidrule{2-9}
...
Simulation-based decision thresholds with a proportion of 5`\%` false positive results were computed for every inference method for all combination of the prior and population effect standard deviation (Figure `\ref{fig:sim_thres}`), per prior standard deviation averaged over all population effect standard deviations (Table `\ref{tab:sim_thres_prior}`), and overall. Note that upper and lower thresholds were set to 2.5`\%` false positive results on each side. With a decision threshold of `r round_and_format(sim_based_thres$savage_dickey_bf[1])`, 5`\%` of Savage Dickey BFs were false positive. The dBF produced 5`\%` false positive results, with a decision threshold of `r round_and_format(sim_based_thres$directional_bf_upper[1])` for a positive and `r round_and_format(sim_based_thres$directional_bf_lower[1])` for a negative effect. A decision rule of P(effect > 0) > `r remove_leading_zero(sim_based_thres$p_effect_upper[1])` for a positive and P(effect > 0) < `r remove_leading_zero(sim_based_thres$p_effect_lower[1])` for a negative effect resulted in 5`\%` false positive results. The HDI against zero decision rule produced 5`\%` false positive results with a `r proportion_to_percentage(sim_based_thres$hdi[1])``\%` HDI.  
...
```
\begin{table}[H]
    \caption{False positive results per decision rule and prior standard deviation}
    \label{tab:false_positives_prior}
    \begin{tabularx}{\textwidth}{lXXXXXXXX}
    \midrule
    ...
    \multicolumn{1}{c}{} & \multicolumn{1}{c}{0.05} & \multicolumn{1}{c}{0.10} & \multicolumn{1}{c}{0.20} & \multicolumn{1}{c}{0.50} & \multicolumn{1}{c}{1.00} & \multicolumn{1}{c}{1.50} & \multicolumn{1}{c}{2.00} & \multicolumn{1}{c}{2.50}\\
    \midrule
    Savage-Dickey BF\textsubscript{10} & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.05,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.1,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.2,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.5,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1.5,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2,"savage_dickey_bf"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2.5,"savage_dickey_bf"])`\arraybackslash\\
    dBF\textsubscript{+-} (upper) & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.05,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.1,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.2,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.5,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1.5,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2,"directional_bf_uppper"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2.5,"directional_bf_uppper"])`\arraybackslash\\
    dBF\textsubscript{+-} (lower) & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.05,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.1,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.2,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.5,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1.5,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2,"directional_bf_lower"])` & \centering `r round_and_format(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2.5,"directional_bf_lower"])`\arraybackslash\\
    P(effect > 0) (upper) & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.05,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.1,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.2,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.5,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1.5,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2,"p_effect_upper")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2.5,"p_effect_upper")`\arraybackslash\\
    P(effect > 0) (lower) & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.05,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.1,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.2,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 0.5,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 1.5,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2,"p_effect_lower")` & \centering `r remove_leading_zero(df_sim_thres_prior[df_sim_thres_prior$prior_sd == 2.5,"p_effect_lower")`\arraybackslash\\
    HDI & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 0.05,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 0.1,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 0.2,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 0.5,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 1,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 1.5,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 2,"hdi")` & \centering `r proportion_to_percentage(df_sim_thres_prior$prior_sd == 2.5,"hdi")`\arraybackslash\\
    \midrule
    \end{tabularx}
\end{table}
```
...
`\subsubsection{Adjustments for multiple tests}`
Simulation-based decision thresholds for multiple tests are shown in Figure `\ref{fig:thresholds}`.`\\`

```
\begin{figure} [H]
    \centering
    \caption{Simulation-based decision thresholds for multiple tests}
...
The prior sensitivity of the Savage-Dickey BF, dBF, and P(effect > 0) was assessed by calculating the median range of the respective inference method between the models within samples. The median range of Savage-Dickey BFs  within samples was `r round_and_format(mwsr_savage_dickey_bf)` (`\textit{IQR}` = `r mwsr_iqr_savage_dickey_bf`), the mean range of dBFs within samples was `r round_and_format(mwsr_iqr_directional_bf)` (`\textit{IQR}` = `r mwsr_directional_bf`), and the mean range of P(effect > 0) proportions within samples was `r round_and_format(mwsr_p_effect)` (`\textit{IQR}` = `r mwsr_iqr_p_effect`).

how to assess the prior sensitivity of HDI against zero decision rule?


`\\`  
To assess the prior sensitivity of the inference methods in terms of the hypothesis decision, the respective proportions of samples for which the decisions of an inference method diverged between models was calculated.

-> sollte unabhängig von der Gesamtanzahl der false positives sein
- Wie viel % der false positive samples sind konsistent (also haben 8 false positive models)?
- False positive models / false positve samples


-> für die 4 empfohlenen decision rules (?)
